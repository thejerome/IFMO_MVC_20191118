    ...
    public ResponseEntity<String> getResult(HttpSession session) {
        try {
            ...
            for (int i = 0; i < equationString.length(); i++) {
                if (Helper.isLetter(equationString.charAt(i))) {
            ...
            return new ResponseEntity<String>(String.valueOf(Helper.calculation(numericEquation.toString())), HttpStatus.valueOf(200));
